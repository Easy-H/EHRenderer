#include "MultiLightShadowShaderClass.hpp"

#include <d3dcompiler.h>

MultiLightShadowShaderClass::MultiLightShadowShaderClass()
{
}

MultiLightShadowShaderClass::MultiLightShadowShaderClass(const MultiLightShadowShaderClass&)
{
}
...
bool MultiLightShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	wchar_t vsFilename[128];
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Shadow/MultiLightShadow.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Shadow/MultiLightShadow.ps") != 0) return false;

	return InitializeShader(device, hwnd, vsFilename, psFilename);
}

void MultiLightShadowShaderClass::Shutdown() {
	ShutdownShader();
}

bool MultiLightShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	...
	ID3D11ShaderResourceView* depthMapTexture2, XMFLOAT3 lightPosition2, XMFLOAT4 diffuseColor2)
{
	...
	if (!SetShaderParameters(deviceContext,
		...
		lightViewMatrix2, lightProjectionMatrix2, depthMapTexture2,
		lightPosition2, diffuseColor2)) return false;

	RenderShader(deviceContext, indexCount);
	return true;
}

bool MultiLightShadowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"MultiLightShadowVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"MultiLightShadowPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
void MultiLightShadowShaderClass::ShutdownShader()
{
}

bool MultiLightShadowShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
...
void MultiLightShadowShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount) {
