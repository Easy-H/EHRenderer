#include "GBufferShaderClass.hpp"
...
GBufferShaderClass::GBufferShaderClass(const GBufferShaderClass&)
{
}
...
bool GBufferShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/GBuffer.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/GBuffer.ps") != 0) return false;
...
bool GBufferShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr, "GBufferVertexShader", "vs_5_0",
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr, "GBufferPixelShader", "ps_5_0",
		D3D10_SHADER_ENABLE_STRICTNESS, 0,
