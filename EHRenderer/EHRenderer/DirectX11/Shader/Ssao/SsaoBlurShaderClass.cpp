#include "SsaoBlurShaderClass.hpp"
...
SsaoBlurShaderClass::SsaoBlurShaderClass(const SsaoBlurShaderClass&)
{
}
...
bool SsaoBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Ssao/SsaoBlur.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Ssao/SsaoBlur.ps") != 0) return false;

	return InitializeShader(device, hwnd, vsFilename, psFilename);
}

bool SsaoBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* ssaoTexture, ID3D11ShaderResourceView* normalDepthTexture,
	int screenWidth, int screenHeight, int blurType)
{
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix,
		ssaoTexture, normalDepthTexture, screenWidth, screenHeight, blurType)) return false;
...
bool SsaoBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr, "SsaoBlurVertexShader", "vs_5_0",
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr, "SsaoBlurPixelShader", "ps_5_0",
	...
	if (!CreateConstantBuffer(device, sizeof(ScreenBufferType),
		_screenBuffer.GetAddressOf())) {
		return false;
	}
...
bool SsaoBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* ssaoTexture, ID3D11ShaderResourceView* normalDepthTexture,
	int screenWidth, int screenHeight, int blurType)
{
	...
	if (FAILED(deviceContext->Map(_screenBuffer.Get(), 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))) {
		return false;
	}

	ScreenBufferType* ptr2 = (ScreenBufferType*)mappedResource.pData;
	...
	deviceContext->Unmap(_screenBuffer.Get(), 0);

	deviceContext->PSSetConstantBuffers(0, 1, _screenBuffer.GetAddressOf());

	deviceContext->PSGetShaderResources(0, 1, &ssaoTexture);
	deviceContext->PSGetShaderResources(1, 1, &normalDepthTexture);
