...
bool FadeShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Fade.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Fade.ps") != 0) return false;
...
bool FadeShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture, float fadeAmount)
{
	if (!SetShaderParameters(deviceContext,
		worldMatrix, viewMatrix, projectionMatrix, texture, fadeAmount)) return false;
...
bool FadeShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"FadeVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"FadePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (!CreateConstantBuffer(device, sizeof(FadeBufferType),
		_fadeBuffer.GetAddressOf())) {
		return false;
	}
...
bool FadeShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	...
	if (FAILED(deviceContext->Map(_fadeBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))) {
		return false;
	}
	...
	deviceContext->Unmap(_fadeBuffer.Get(), 0);

	deviceContext->PSSetConstantBuffers(0, 1, _fadeBuffer.GetAddressOf());
