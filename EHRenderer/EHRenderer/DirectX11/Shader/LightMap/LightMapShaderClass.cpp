#include "LightMapShaderClass.hpp"
...
bool LightMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/LightMap.vs") != 0) {
		return false;
	}

	wchar_t psFilename[128];

	if (wcscpy_s(psFilename, 128, L"./HLSL/LightMap.ps") != 0) {
		return false;
	}
...
bool LightMapShaderClass::InitializeShader(ID3D11Device* device,
	HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"LightMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"LightMapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool LightMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	...
	if (FAILED(deviceContext->Map(_matrixBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0,
		&mappedResource)))
	{
		return false;
	}
