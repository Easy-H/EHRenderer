#include "ParticleShaderClass.hpp"
...
bool ParticleShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Particle.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Particle.ps") != 0) return false;
...
bool ParticleShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
...
bool ParticleShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"ParticleVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"ParticlePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool ParticleShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture) {
