...
BlurShaderClass::BlurShaderClass(const BlurShaderClass&)
{
}
...
bool BlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Blur.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Blur.ps") != 0) return false;
	...
	ID3D11ShaderResourceView* texture, int screenWidth, int screenHeight, float blurType)
{
	if (!SetShaderParameters(deviceContext,
		...
		screenWidth, screenHeight, blurType)) return false;
...
bool BlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"BlurVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"BlurPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(device->CreateBuffer(&screenBufferDesc, nullptr, _screenBuffer.GetAddressOf()))) {
		return false;
	}
	...
	ID3D11ShaderResourceView* texture, int screenWidth, int screenHeight, float blurType)
{
	...
	ScreenBufferType* dataPtr2 = (ScreenBufferType*)mappedResource.pData;
