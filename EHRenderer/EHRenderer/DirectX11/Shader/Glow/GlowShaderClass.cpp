...
GlowShaderClass::GlowShaderClass(const GlowShaderClass&)
{
}
...
bool GlowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Glow.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Glow.ps") != 0) return false;
...
bool GlowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* glowTexture, float glowStrength)
{
	if (!SetShaderParameters(deviceContext,
		worldMatrix, viewMatrix, projectionMatrix, texture, glowTexture, glowStrength)) return false;
...
bool GlowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"GlowVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"GlowPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (!CreateConstantBuffer(device, sizeof(GlowBufferType),
		_glowBuffer.GetAddressOf())) {
		return false;
	}
...
bool GlowShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* glowTexture, float glowStrength) {
	...
	if (FAILED(deviceContext->Map(_glowBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))) {
		return false;
	}
	...
	deviceContext->Unmap(_glowBuffer.Get(), 0);

	deviceContext->PSSetConstantBuffers(bufferNumber, 1, _glowBuffer.GetAddressOf());
