#include "TranslateShaderClass.hpp"
...
bool TranslateShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Translate.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Translate.ps") != 0) return false;
...
bool TranslateShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	...
	ID3D11ShaderResourceView* Translate, float translation)
{
	if (!SetShaderParameters(deviceContext,
		worldMatrix, viewMatrix, projectionMatrix, Translate, translation)) return false;
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"TranslateVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"TranslatePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (!CreateConstantBuffer(device, sizeof(TranslateBufferType),
		_translateBuffer.GetAddressOf())) {
		return false;
	}
...
bool TranslateShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	...
	if (FAILED(deviceContext->Map(_translateBuffer.Get(), 0,
	...
	deviceContext->PSSetConstantBuffers(0, 1,_translateBuffer.GetAddressOf());
