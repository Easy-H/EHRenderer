...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	if (wcscpy_s(vsFilename, 128, L"./HLSL/Water.vs") != 0) return false;

	wchar_t psFilename[128];
	if (wcscpy_s(psFilename, 128, L"./HLSL/Water.ps") != 0) return false;
	...
	ID3D11ShaderResourceView* normalTexture, float waterTranslation, float reflectRefractScale)
{
	if (!SetShaderParameters(deviceContext,
		...
		waterTranslation, reflectRefractScale)) return false;
...
bool WaterShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(D3DCompileFromFile(vsFilename, nullptr, nullptr,
		"WaterVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	if (FAILED(D3DCompileFromFile(psFilename, nullptr, nullptr,
		"WaterPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	ID3D11ShaderResourceView* normalTexture, float waterTranslation, float reflectRefractScale) 
{
	...
	if (FAILED(deviceContext->Map(_reflectionBuffer.Get(), 0,
	...
	deviceContext->Unmap(_reflectionBuffer.Get(), 0);
	deviceContext->VSSetConstantBuffers(1, 1, _reflectionBuffer.GetAddressOf());


	if (FAILED(deviceContext->Map(_waterBuffer.Get(), 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))) {
		return false;
	}
	...
	deviceContext->Unmap(_waterBuffer.Get(), 0);

	deviceContext->PSSetConstantBuffers(0, 1,_waterBuffer.GetAddressOf());
